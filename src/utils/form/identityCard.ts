 ...
 *
 * @author dyb-dev
 * @date 2024-10-15 16:26:43
 * @param {string} identityCardNumber 身份证号码
 * @returns {*}  {boolean} 是否为有效的省、直辖市代码
 */
const isProvinceCode = (identityCardNumber: string): boolean => {
 ...
 */
const isBirthday = (identityCardNumber: string): boolean => {
 ...
 *
 * @author dyb-dev
 * @date 2024-10-15 18:09:43
 * @param {string} identityCardNumber 身份证号码
 * @returns {*}  {boolean} 是否为有效的校验位
 */
const isParityBit = (identityCardNumber: string): boolean => {
...
export const isIdentityCard = (identityCardNumber: string): boolean => {
    ...
    return _check && isProvinceCode(identityCardNumber) && isBirthday(identityCardNumber) && isParityBit(identityCardNumber)
